...
class ContextUpdaterTests(unittest.IsolatedAsyncioTestCase):
    ...
    # ------------------------------------------------------------------
    async def test_no_op_when_both_flags_disabled(self) -> None:
        ...
        await refresh_system_prompt(ctx, orchestrator, "sess1", settings)
        self.assertEqual(ctx.messages[0]["content"], "original")

    # ------------------------------------------------------------------
    # injects system prompt into empty context
    # ------------------------------------------------------------------
    async def test_creates_system_message_in_empty_context(self) -> None:
        ctx = _FakeLLMContext([])
    ...
    # ------------------------------------------------------------------
    async def test_updates_existing_system_message(self) -> None:
    ...
    # ------------------------------------------------------------------
    async def test_inserts_system_before_user_messages(self) -> None:
        ctx = _FakeLLMContext([
    ...
    # ------------------------------------------------------------------
    async def test_progress_block_included_in_prompt(self) -> None:
    ...
    # ------------------------------------------------------------------
    async def test_progress_block_skipped_when_inject_off(self) -> None:
        ...
        await refresh_system_prompt(ctx, orchestrator, "sess1", settings, memory_hub=mock_hub)
        self.assertNotIn(progress_text, ctx.messages[0]["content"])

    # ------------------------------------------------------------------
    # memory block integration (memory_inject=True)
    # ------------------------------------------------------------------
    async def test_memory_block_included_when_inject_on(self) -> None:
        ctx = _FakeLLMContext([{"role": "system", "content": "original"}])
    ...
    # ------------------------------------------------------------------
    async def test_no_crash_when_memory_hub_is_none(self) -> None:
        ctx = _FakeLLMContext([{"role": "system", "content": "orig"}])
        orchestrator = _FakeOrchestrator("progress")
        settings = self._settings(progress_inject=True, memory_inject=True)
        await refresh_system_prompt(ctx, orchestrator, "sess1", settings, memory_hub=None)
        self.assertEqual(ctx.messages[0]["role"], "system")

    # ------------------------------------------------------------------
    # Preserve history messages after system message
    # ------------------------------------------------------------------
    async def test_history_messages_preserved_after_refresh(self) -> None:
        ctx = _FakeLLMContext([
